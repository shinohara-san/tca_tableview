...
class BookListViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let isShowingAlertPublisher = self.viewStore.publisher
            .map(\.isShowingAlert)
        let errorMessagePublisher = self.viewStore.publisher.map(\.errorMessage)

        isShowingAlertPublisher
            .zip(errorMessagePublisher)
            .filter(\.0)
            .sink { [weak self] _, errorMessage in
